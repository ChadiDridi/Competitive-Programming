...
void solve(int tc){  //the number of ways we can decode number where each digit is an english alphabet from 1 to 26 
    f0r(i,n){
        cin>>y;
        a[i]=y; 
    }
    ...
    for(ll i=n-1;i>=0;i--){
        if(a[i]==0){
            ...
            b[i]=b[i+1];
            if(i<n-1 && (a[i]*10+a[i+1])<=26){
                    b[i]+=b[i+2];
                
            }
        }
    }
    cout<<dp[0]<<endl;  //O(n time and O(n) space)
}
int main(){
    #ifdef chadi_local 
